...
if(isset($_SESSION['id_pers']) && valider("connecte", "SESSION")) {
    // Récupérer le rôle de l'utilisateur (par exemple, depuis une fonction recupRole)
    $role = recupRole($_SESSION['id_pers']);

    // Vérifier si l'utilisateur a un rôle différent de 3 (ou tout autre rôle sensible)
    if ($role == "1" || $role == "2") {
        // Vérifier si l'utilisateur tente d'accéder à une vue sensible
        if (in_array($view, $vuesSensibles)) {
	...
	if ($role == "2") {
		// Vérifier si l'utilisateur tente d'accéder à une vue sensible
		if (in_array($view, $vuesSensibles2)) {
			// Rediriger vers la page d'accueil
			header("Location: index.php");
			exit;
		}
