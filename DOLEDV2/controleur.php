		...
		case "creerMarque":
			...
			case "creerCategorie":
				...
				if (empty($nom_categorie) || empty($descriptif) || empty($image)) {
					...
					if($sousCat==-1) $nouvelleCatId = creer_categorieMere($nom_categorie, $descriptif, $image);
					else $nouvelleCatId = creer_categorie($nom_categorie, $descriptif, $image, $sousCat);
					// Utilisez $nouvelleMarqueId comme nécessaire ici
